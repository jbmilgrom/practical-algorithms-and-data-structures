...
def binary_search(sortedList, item):
  ...
  if length == 0:
    return False
  return iterate_binary_search(sortedList, item, 0, length - 1)

# @private
def iterate_binary_search(sortedList, item, start, finish):
  if start == finish:
  ...
  if item < marker:
    ...
    #   from the next slice
    return iterate_binary_search(sortedList, item, start, mid - 1)
  else:
    return iterate_binary_search(sortedList, item, mid, finish)
